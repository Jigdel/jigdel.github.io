...
def plot_weather_data(turnstile_weather):
    ...
    '''
    # Scatter Plot - Hourly entry traffic with/without rain
    p = ggplot(turnstile_weather, aes('Hour', 'ENTRIESn_hourly', color = 'rain'))
    limit_axes = xlim(-.75, 24.5) + ylim(0, 48000)
    plot = p + geom_point() + labs('Hour', 'Entries') + limit_axes
    '''
    '''
    # Scatter Plot - Hourly entry traffic based on stations
    p = ggplot(turnstile_weather, aes('Hour', 'ENTRIESn_hourly', color = 'UNIT'))
    limit_axes = xlim(-.75, 24.5) + ylim(0, 48000)
    plot = p + geom_point() + labs('Hour', 'Entries') + limit_axes
    '''
    ...
    '''
    # Plot - Hourly entry traffic
    p = ggplot(turnstile_weather, aes('Hour', 'ENTRIESn_hourly'))
    plot = p + geom_line(color = 'red') + labs('Hour', 'Entries')
    '''
