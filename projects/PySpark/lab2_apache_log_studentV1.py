...
#not200 = access_logs.map(lambda log: (log.endpoint if (log.response_code != 200) else None))
not200 = access_logs.filter(lambda log: (log.response_code != 200))
not200endpoint = not200.map(lambda l: l.endpoint)
#print not200endpoint.take(15)

#endpointCountPairTuple = not200.map(lambda l: (l, 1))
#print endpointCountPairTuple.take(10)

endpointCountPairTuple = not200endpoint.map(lambda l: (l, 1))
#print endpointCountPairTuple.take(10)

endpointSum = endpointCountPairTuple.reduceByKey(lambda a, b : a + b).cache()

#extract the top ten endpoints by performing a takeOrdered with a value of 10 
#and a lambda function that multiplies the count (the second element of 
#each pair) by -1 to create a sorted list with the top endpoints at the 
#bottom.
...
#print dayToHostPairTuple.take(10)

dayGroupedHosts = dayToHostPairTuple.groupByKey()
#print dayGroupedHosts.take(3)

#dayHostCount = dayGroupedHosts.map(lambda a, b : (a, len(b)))
dayHostCount = dayGroupedHosts.mapValues(set) #(list),(len)
#dayHostCount = dayGroupedHosts.map(lambda a, b : (a, set(b)))
#print dayHostCount.take(2)
'''
dailyHosts = (dayHostCount
              .map(lambda a, b : (a, len(b))
              .cache())
#print dailyHosts.take(2)
...
badRecords = (access_logs
              .map(lambda log: log.response_code)
              .filter(lambda l: l.response_code==404)
              .cache())
print 'Found %d 404 URLs' % badRecords.count()

'''
badRecords = (access_logs
              .map(lambda log: log.response_code==404)
              .filter(lambda l: l==True)
              .cache())
print 'Found %d 404 URLs' % badRecords.count()
'''
