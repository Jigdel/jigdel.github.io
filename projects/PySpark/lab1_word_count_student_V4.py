...
# wordsRDD.groupByKey().map(lambda (k, v): (k, v))
wordsGrouped = wordPairs.groupByKey().map(lambda (k, v): (k, v))
...
def wordCount(wordListRDD):
    ...
    """
    w_count = wordListRDD.map(lambda x: (x, 1)).reduceByKey(add)
    return w_count
...
def removePunctuation(text):
    ...
    # remove punctuation
    text_punc_removed = re.sub('([^A-Za-z0-9 ]+)', '', text)
    #return text_punc_removed
    
    # remove leading/trailing space
    text_no_leadtrail_space = re.sub(r'^\s+|\s+$', '', text_punc_removed)
    # s/^\s+|\s+$//g # both sides
    # s/\s+$//; # right trim
    return text_no_leadtrail_space.lower()

    # lower case - overcomplicating things (lesson learnt!)
    #text_lower = re.sub(r'([A-Za-z]){1}', lambda pat: pat.group().lower(), text_punc_removed)
...
# TODO: Replace <FILL IN> with appropriate code
shakespeareWordsRDD = shakespeareRDD.flatMap(lambda x: x.split(' ')) #flatMap(lambda x: x.strip().split())
...
# TODO: Replace <FILL IN> with appropriate code
# Note to self: used to wrong RDD (shakespeareWordsRDD instead of shakeWordsRDD)
